...
6. Assessment / Medical Assessment: This is the most critical section. Write it as FLOWING CLINICAL PROSE — one cohesive narrative, NOT lettered sub-points or sub-headings. Do NOT use a., b., c. formatting or bold sub-headers in the output. Instead, weave together ALL of the following into a single authoritative clinical narrative:
   - Synthesize subjective complaints and objective findings into a cohesive clinical picture, explaining how findings relate to each other
   - State the most likely diagnosis first, then note plausible differentials with the specific documented findings that support or argue against each — all strictly from the JSON data
   - Quantify severity using documented measurements and relate impairments to functional limitations
   - Note prognostic indicators from documented data (chronicity, aggravating/alleviating factors, prior treatment response, patient goals)
   - ALL reasoning must trace directly to facts in the EncounterState JSON — never introduce diagnoses or clinical patterns not supported by documented data
   Example style: "Findings of limited shoulder flexion ROM to 120° with end-range pain, combined with positive Neer's and Hawkins-Kennedy tests, are consistent with subacromial impingement syndrome. Differential considerations include rotator cuff tendinopathy (supported by painful arc and weakness with resisted ER) and adhesive capsulitis (less likely given preserved passive ROM). Grip strength deficit of 40% vs contralateral side limits the patient's ability to perform sustained-grip occupational tasks. Chronicity of 6 weeks with progressive worsening suggests a guarded short-term prognosis, though patient's stated goal of full return to work is a favorable motivating factor."
